import json
from etl.db import PostgresDB
from etl.ingest import IngestPipeline

def write_audit(actor, action, batch_id=None, details=None, severity="INFO"):
    PostgresDB.execute(
        """
        INSERT INTO audit_log(actor, action, batch_id, details, severity)
        VALUES (%s, %s, %s, %s, %s)
        """,
        (actor, action, batch_id, json.dumps(details or {}), severity)
    )

if __name__ == "__main__":
    PostgresDB.initialize()
    try:
        results = IngestPipeline.ingest_sources()
        write_audit("etl_runner", "run_ingest", None, {"batches_created": len(results)})
    except Exception as e:
        write_audit("etl_runner", "run_ingest_failed", None, {"error": str(e)}, "ERROR")
        raise
